...
def convert(path_to_src_dir: str, path_to_dst_dir: str):
    ...
    path_to_meta_file = os.path.join(path_to_src_dir, 'meta')
    with open(path_to_meta_file, 'rb') as f:
        meta_dict = pickle.load(f, encoding='bytes')
    for idx, label_name in enumerate(meta_dict[b'fine_label_names']):
        cifar_class_to_category_dict[idx] = label_name.decode(encoding='utf-8')

    for phase in ['train', 'test']:
        path_to_phase_file = os.path.join(path_to_src_dir, phase)
        with open(path_to_phase_file, 'rb') as f:
            cifar_dict = pickle.load(f, encoding='bytes')

        for image_data, label in tqdm(zip(cifar_dict[b'data'], cifar_dict[b'fine_labels']),
                                      desc=f'Converting phase {phase}'):
