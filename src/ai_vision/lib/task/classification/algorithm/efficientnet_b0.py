...
class EfficientNet_B0(Algorithm):
    ...
    def _build_net(self) -> nn.Module:
        ...
        # x = efficientnet_b0._conv_stem(x); print(x.shape)
        # x = efficientnet_b0._bn0(x); print(x.shape)
        # x = efficientnet_b0._swish(x); print(x.shape)
        # for i in range(len(efficientnet_b0._blocks)):
        #     x = efficientnet_b0._blocks[i](x); print(i, x.shape)
        conv1 = nn.ModuleList([efficientnet_b0._conv_stem, efficientnet_b0._bn0, efficientnet_b0._swish] +
        ...
        conv5 = nn.ModuleList(list(efficientnet_b0._blocks[11:]) +
                              [efficientnet_b0._conv_head, efficientnet_b0._bn1, efficientnet_b0._swish])
