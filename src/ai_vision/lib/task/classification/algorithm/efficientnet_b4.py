...
class EfficientNet_B4(Algorithm):
    ...
    def _build_net(self) -> nn.Module:
        if self.pretrained:
            efficientnet_b4 = EfficientNet.from_pretrained(model_name='efficientnet-b4')
        else:
            efficientnet_b4 = EfficientNet.from_name(model_name='efficientnet-b4')

        efficientnet_b4._fc = nn.Linear(in_features=efficientnet_b4._fc.in_features, out_features=self.num_classes)

        # x = torch.randn(1, 3, 600, 1000)
        # x = efficientnet_b4._conv_stem(x); print(x.shape)
        # x = efficientnet_b4._bn0(x); print(x.shape)
        # x = efficientnet_b4._swish(x); print(x.shape)
        # for i in range(len(efficientnet_b4._blocks)):
        #     x = efficientnet_b4._blocks[i](x); print(i, x.shape)
        conv1 = nn.ModuleList([efficientnet_b4._conv_stem, efficientnet_b4._bn0, efficientnet_b4._swish] +
                              list(efficientnet_b4._blocks[:2]))
        conv2 = efficientnet_b4._blocks[2:6]
        conv3 = efficientnet_b4._blocks[6:10]
        conv4 = efficientnet_b4._blocks[10:22]
        conv5 = nn.ModuleList(list(efficientnet_b4._blocks[22:]) +
                              [efficientnet_b4._conv_head, efficientnet_b4._bn1, efficientnet_b4._swish])
