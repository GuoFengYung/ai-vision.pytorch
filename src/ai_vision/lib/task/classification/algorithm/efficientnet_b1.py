...
class EfficientNet_B1(Algorithm):
    ...
    def _build_net(self) -> nn.Module:
        if self.pretrained:
            efficientnet_b1 = EfficientNet.from_pretrained(model_name='efficientnet-b1')
        else:
            efficientnet_b1 = EfficientNet.from_name(model_name='efficientnet-b1')

        efficientnet_b1._fc = nn.Linear(in_features=efficientnet_b1._fc.in_features, out_features=self.num_classes)

        # x = torch.randn(1, 3, 600, 1000)
        # x = efficientnet_b1._conv_stem(x); print(x.shape)
        # x = efficientnet_b1._bn0(x); print(x.shape)
        # x = efficientnet_b1._swish(x); print(x.shape)
        # for i in range(len(efficientnet_b1._blocks)):
        #     x = efficientnet_b1._blocks[i](x); print(i, x.shape)
        conv1 = nn.ModuleList([efficientnet_b1._conv_stem, efficientnet_b1._bn0, efficientnet_b1._swish] +
                              list(efficientnet_b1._blocks[:1]))
        conv2 = efficientnet_b1._blocks[1:5]
        conv3 = efficientnet_b1._blocks[5:8]
        conv4 = efficientnet_b1._blocks[8:16]
        conv5 = nn.ModuleList(list(efficientnet_b1._blocks[16:]) +
                              [efficientnet_b1._conv_head, efficientnet_b1._bn1, efficientnet_b1._swish])
