...
class Analyzer:
    ...
    def analyze(self, model: Model, lower_prob_thresh: float, upper_prob_thresh: float):
        ...
        while True:
            ...
            process_dict = prediction.image_id_to_process_dict_dict[image_id]
            ...
            pred_bboxes = [prediction.sorted_all_pred_bboxes[index] for index in indices]
            ...
            pred_probmasks = [prediction.sorted_all_pred_probmasks[index] for index in indices]
            ...
            gt_masks = prediction.image_id_to_gt_masks_dict[image_id]
            difficulties = prediction.image_id_to_difficulties_dict[image_id].tolist()

            if len(pred_bboxes) > 0:
                pred_bboxes = Preprocessor.inv_process_bboxes(process_dict,
                                                              torch.stack(pred_bboxes, dim=0)).tolist()

            if len(pred_probmasks) > 0:
                pred_probmasks = Preprocessor.inv_process_probmasks(process_dict,
                                                                    torch.stack(pred_probmasks, dim=0))
            ...
            if needs_show_pred_mask:
                for color, probmask in enumerate(pred_probmasks, start=1):
            ...
            if needs_show_gt_mask:
                for color, mask in enumerate(gt_masks, start=1):
                    mask_image = to_pil_image(mask * color)
                    mask_image.putpalette(flatten_palette)
                    blended_image = Image.blend(image.convert('RGBA'), mask_image.convert('RGBA'), alpha=0.5).convert('RGB')
                    image = Image.composite(blended_image, image, mask=to_pil_image(mask * 255).convert('1'))
            ...
            if key == 27:
                ...
                needs_show_pred_mask = not needs_show_pred_mask
            elif key == ord('g'):
