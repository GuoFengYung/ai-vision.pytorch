...
def _upgrade(task_name: Task.Name, path_to_checkpoints_dir: str):
    ...
    if task_name == Task.Name.CLASSIFICATION:
        from aibox_vision.lib.task.classification.upgrader import Upgrader
    elif task_name == Task.Name.DETECTION:
        from aibox_vision.lib.task.detection.upgrader import Upgrader
    elif task_name == Task.Name.INSTANCE_SEGMENTATION:
        from aibox_vision.lib.task.instance_segmentation.upgrader import Upgrader
    else:
        raise ValueError
