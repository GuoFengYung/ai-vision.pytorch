...
from aibox_vision.lib.logger import Logger
from aibox_vision.lib.task import Task
from aibox_vision.lib.task.classification.algorithm import Algorithm as ClassificationAlgorithm
from aibox_vision.lib.task.classification.config import Config as ClassificationConfig
from aibox_vision.lib.task.detection.algorithm import Algorithm as DetectionAlgorithm
...
from aibox_vision.lib.task.instance_segmentation.algorithm import Algorithm as InstanceSegmentationAlgorithm
from aibox_vision.lib.task.instance_segmentation.config import Config as InstanceSegmentationConfig
from aibox_vision.lib.util import Util
...
def _train(config: Config, terminator: Event):
    ...
    if config.task_name == Task.Name.CLASSIFICATION:
        from aibox_vision.lib.task.classification.db import DB as T
        db_class = T
    elif config.task_name == Task.Name.DETECTION:
        from aibox_vision.lib.task.detection.db import DB as T
        db_class = T
    elif config.task_name == Task.Name.INSTANCE_SEGMENTATION:
        from aibox_vision.lib.task.instance_segmentation.db import DB as T
        db_class = T
    else:
        raise ValueError
    ...
    try:
        ...
        if config.task_name == Task.Name.CLASSIFICATION:
            from aibox_vision.lib.task.classification.trainer import Trainer
            assert isinstance(config, ClassificationConfig)
        elif config.task_name == Task.Name.DETECTION:
            from aibox_vision.lib.task.detection.trainer import Trainer
            assert isinstance(config, DetectionConfig)
        elif config.task_name == Task.Name.INSTANCE_SEGMENTATION:
            from aibox_vision.lib.task.instance_segmentation.trainer import Trainer
