...
class EfficientNet_B6(Algorithm):
    ...
    def _build_net(self) -> nn.Module:
        if self.pretrained:
            efficientnet_b6 = EfficientNet.from_pretrained(model_name='efficientnet-b6')
        else:
            efficientnet_b6 = EfficientNet.from_name(model_name='efficientnet-b6')

        efficientnet_b6._fc = nn.Linear(in_features=efficientnet_b6._fc.in_features, out_features=self.num_classes)

        # x = torch.randn(1, 3, 600, 1000)
        # x = efficientnet_b6._conv_stem(x); print(x.shape)
        # x = efficientnet_b6._bn0(x); print(x.shape)
        # x = efficientnet_b6._swish(x); print(x.shape)
        # for i in range(len(efficientnet_b6._blocks)):
        #     x = efficientnet_b6._blocks[i](x); print(i, x.shape)
        conv1 = nn.ModuleList([efficientnet_b6._conv_stem, efficientnet_b6._bn0, efficientnet_b6._swish] +
                              list(efficientnet_b6._blocks[:3]))
        conv2 = efficientnet_b6._blocks[3:9]
        conv3 = efficientnet_b6._blocks[9:15]
        conv4 = efficientnet_b6._blocks[15:31]
        conv5 = nn.ModuleList(list(efficientnet_b6._blocks[31:]) +
                              [efficientnet_b6._conv_head, efficientnet_b6._bn1, efficientnet_b6._swish])
