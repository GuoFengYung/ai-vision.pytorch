...
class EfficientNet_B5(Algorithm):
    ...
    def _build_net(self) -> nn.Module:
        if self.pretrained:
            efficientnet_b5 = EfficientNet.from_pretrained(model_name='efficientnet-b5')
        else:
            efficientnet_b5 = EfficientNet.from_name(model_name='efficientnet-b5')

        efficientnet_b5._fc = nn.Linear(in_features=efficientnet_b5._fc.in_features, out_features=self.num_classes)

        # x = torch.randn(1, 3, 600, 1000)
        # x = efficientnet_b5._conv_stem(x); print(x.shape)
        # x = efficientnet_b5._bn0(x); print(x.shape)
        # x = efficientnet_b5._swish(x); print(x.shape)
        # for i in range(len(efficientnet_b5._blocks)):
        #     x = efficientnet_b5._blocks[i](x); print(i, x.shape)
        conv1 = nn.ModuleList([efficientnet_b5._conv_stem, efficientnet_b5._bn0, efficientnet_b5._swish] +
                              list(efficientnet_b5._blocks[:3]))
        conv2 = efficientnet_b5._blocks[3:8]
        conv3 = efficientnet_b5._blocks[8:13]
        conv4 = efficientnet_b5._blocks[13:27]
        conv5 = nn.ModuleList(list(efficientnet_b5._blocks[27:]) +
                              [efficientnet_b5._conv_head, efficientnet_b5._bn1, efficientnet_b5._swish])
